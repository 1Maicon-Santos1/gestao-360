...
import { Plus, Search, MoreHorizontal, Edit, Trash2, CheckCircle2, AlertCircle, Clock, TrendingDown, X } from 'lucide-react'
...
export default function PayablesPage() {
  ...
  const { register, handleSubmit, reset, watch, setValue, formState: { errors, isSubmitting } } = useForm<FormData>({
    ...
    defaultValues: { context_type: 'personal', status: 'pending', competency_date: TODAY, amount: 0 },
  })

  const openCreate = () => {
    reset({ context_type: 'personal', status: 'pending', competency_date: TODAY, amount: 0 })
    setEditing(null)
    setDialogOpen(true)
  }
  ...
  const openEdit = (t: Transaction) => {
    ...
    reset({
      ...
      responsible_person_id: t.responsible_person_id, notes: t.notes,
    })
    setDialogOpen(true)
  }
  ...
  const onSubmit = async (data: FormData) => {
    const payload = {
      ...data, transaction_type: 'expense' as const,
      ...
      subcategory_id: null, supplier_id: null, client_id: null, cost_center_id: null,
  ...
  const totals = {
    ...
    paid: filtered.filter((t) => t.status === 'paid').reduce((s, t) => s + t.amount, 0),
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Controle seus pagamentos e despesas pendentes"
        action={
        ...
        <Card className="bg-red-50 border-red-200">
          <CardContent className="pt-4 pb-3 px-4">
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs text-red-700 font-medium">Atrasado</p>
            </div>
        ...
        <Card className="bg-green-50 border-green-200">
          <CardContent className="pt-4 pb-3 px-4">
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs text-green-700 font-medium">Pago este mês</p>
            </div>
            <p className="text-lg font-bold text-green-700 mt-1 tabular-nums">{formatCurrency(totals.paid)}</p>
          </CardContent>
        </Card>
      </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Adicione suas contas a pagar para não perder nenhum vencimento."
          action={{ label: 'Nova Conta a Pagar', onClick: openCreate }}
        />
      ) : (
            ...
            return (
              ...
              >
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-1">
                    ...
                    {t.category && (
                      <div className="flex items-center gap-1">
                        <div className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: t.category.color }} />
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <span className="font-bold text-base tabular-nums text-red-600">{formatCurrency(t.amount)}</span>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      {t.status === 'pending' && (
                        <DropdownMenuItem onClick={() => markPaid.mutate({ id: t.id, type: 'expense' })} className="gap-2">
                          <CheckCircle2 className="h-4 w-4 text-green-500" /> Marcar como Pago
                        </DropdownMenuItem>
                      )}
              ...
              <div className="space-y-1.5">
                ...
                <Select value={watch('category_id') ?? 'none'} onValueChange={(v) => setValue('category_id', v === 'none' ? null : v)}>
                  ...
                  <SelectContent>
                    ...
                    {categories.filter((c) => c.category_type === 'expense').map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
      ...
      <ConfirmDialog
        ...
        title="Excluir conta a pagar?" description="Esta ação não pode ser desfeita."
